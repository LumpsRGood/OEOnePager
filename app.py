...
def save_classifications_merge(updates_df):
    """Reliable merge that actually writes back."""
    try:
        ws = open_ws()
        ...
        # Write in chunks to prevent silent API truncation
        ws.clear()
        ws.resize(len(merged) + 1, 2)
        header = [["Opportunity", "Classification"]]
        data_rows = merged.values.tolist()
        ws.update("A1", header + data_rows, value_input_option="RAW")

        check = ws.get_all_values()
        st.success(f"✅ {len(check)-1} rows saved to Google Sheet.")
        return merged
